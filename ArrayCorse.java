...
public class ArrayCorse {
	...
	public int modificaOrari_giornalieri(String nome_corsa, double nuovoValoreOrari) 
	{
		for(int i=0;i<ArrayCorse.MAX_NUM_CORSE;i++)
		{
			if(lista_corse[i].getNome_corsa().equals(nome_corsa))
			{	    
				lista_corse[i].setOrari_giornalieri(nuovoValoreOrari);
				return i; //restituisce la posizione libera orari_giornalieri cancellata logicamente
			}
		}		
	  return -1; //Gli orari giornalieri NON sono presenti
	}
	//metodo numero corse
